...
# check root
[[ $EUID -ne 0 ]] && echo -e "${red}mistake：${plain} the script must run with root user！\n" && exit 1
...
if [[ $arch == "x86_64" || $arch == "x64" || $arch == "amd64" ]]; then
    ...
    arch="amd64"
    echo -e "${red}Schema failed detected，Use the default schema: ${arch}${plain}"
fi
...
config_after_install() {
    ...
    if [[ x"${config_confirm}" == x"y" || x"${config_confirm}" == x"Y" ]]; then
        read -p "Please set your account name to login:" config_account
        ...
        echo -e "${yellow}Please confirm the settings${plain}"
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red}ailed to detect the x-ui version, it may be beyond the limit of Github API, please try again later, or manually specify the x-ui version to install${plain}"
            exit 1
        fi
    ...
    config_after_install
    #echo -e "If it is a new installation, the default web port is ${green}54321${plain}，The default username and password are ${green}admin${plain}"
    #echo -e "Please make sure this port is not occupied by other programs，${yellow}and make sure 54321 Port has been released${plain}"
    ...
    #echo -e ""
    #echo -e "If you areupdating the panel, access to the panel as you did before!"
    #echo -e ""
    ...
    echo -e "x-ui enable       - enable x-ui at boot"
    echo -e "x-ui disable      - disable x-ui autostart"
    echo -e "x-ui log          - View x-ui logs"
    echo -e "x-ui v2-ui        - use v2-ui account info in x-ui in this machine"
